...
class Gu:
    ...
    def set_demand(self):   #dest의 각 구마다 demand 따로 넣고, percent 정리, priority 정리, OD, PD 정리
        ...
        self.demands = []
        self.percents = []
        demand_leng = [] # set_priority에서 구들의 수요 수 계산해서 가중치 계산할 때 쓰려고
        demands_length = len(self.demand)   # demand 아예 없는 경우도 대비
        for i in range(numsection):
            if demands_length>0:
                # print('self.demand {}'.format(self.demand.shape))
                tmp_demand = self.demand[self.demand[:,1]==section_dict[i]]
                demand_leng.append(len(tmp_demand)) # for priority
                # print(demands_length)
                if tmp_demand.size != 0:
                    self.demands.append(tmp_demand)
                else:
                    self.demands.append(np.array([]))
            else:
                demand_leng.append(0)
                self.demands.append(np.array([]))
        for i in range(numsection):
            if demands_length != 0 :
                self.percents.append(len(self.demands[i]) / demands_length)
            else:
        ...
        self.OD_distances = []
        self.PD_distances = []
        # print('demand length {}'.format(len(self.demands)))

        for i in range(numsection):
            # print('demands[i] length {}'.format(len(self.demands[i])))

            self.OD_distances.append(get_od(self.demands[i]))
            # print('set demand : self.supply {} self.demands[i] {}'.format(self.supply, self.demands[i]))
            self.PD_distances.append(get_pd(self.supply, self.demands[i]))

        self.set_priority(0.5, 0.5, demand_leng)

    def set_priority(self, a, b, num_demand):
        global section_dict, numsection
        average = [ [0,i] for i in range(numsection)]
        mean_OD = []
        mean_demand = []
        for i in range(numsection):
            mean_OD.append( [np.sum(self.OD_distances[i]) / len(self.OD_distances[i]), i] )
            mean_demand.append( [num_demand[i], i] )
        mean_OD.sort(reverse=True)
        mean_demand.sort(reverse=True)
        for i in range(numsection):
            average[mean_OD[i][1]][0] += mean_OD[i][0] * a
            average[mean_demand[i][1]][0] += mean_demand[i][0] * b
        average.sort(reverse=True)
        for i in range(numsection):
            self.priority[i] = average[i][1]  # priority[0] -> 4, [1] -> 3... 해당구의 index
    ...
    def get_action(self, state):
        ...
        return env.DQN_agent.get_action(state)
    ...
    def matching(self, action, is_sim = True): # reward, new supply return
        ...
        total_supply_len = len(self.supply)
        sum_matched_demand = 0
        unmatched_demand = []
        ##################################### matching 시작 #####################################

        for i in range(numsection):
            if len(self.PD_distances[self.priority[i]]) == 0:
                continue
            matched_demand = []
            tmp_demand = self.demands[self.priority[i]]
            tmp_od = self.OD_distances[self.priority[i]]
            tmp_pd = self.PD_distances[self.priority[i]]
            tmp_pd[:,matched_supply] = np.max(tmp_pd)    # matching된 supply들 PD 0으로
            matching_matrix = 1 - tmp_pd/np.max(tmp_pd)
            section_supply_len = int(percents[self.priority[i]]*total_supply_len)
            ...
            sum_matched_demand += len(matched_demand)
...
class Platform:  # 역할: OD별, PD별로 demand, supply 정리해서 gu에 넘겨주기, simulation, matching
    def __init__(self, case):
        ...
        global state_size
        state_size = 4 * self.numsection + 1
        self.DQN_agent = DQN_agent

        # for i in range(self.numsection):
        #     self.gu_list.append(Gu(i))

        global numsection, section_dict, second
    ...
    def get_state(self, section_idx):    #DQN 돌리기 위해서 state 만들어주기 (list 형태로 반환)
        total_demand = 0
        for i in range(self.numsection):
            ...
            for j in range(numsection):
                ...
                if i > 0:
                    if i == self.episode_time - 1:
                        self.DQN_agent.append_sample(old_state[j], best_actions[j], r, new_state[j], True)
                        self.DQN_agent.update_target_model()
                    else:
                        self.DQN_agent.append_sample(old_state[j], best_actions[j], r, new_state[j], False)
                if len(self.DQN_agent.memory) >= self.DQN_agent.train_start:
                    self.DQN_agent.train_model()
...
if __name__ == "__main__":
    ...
    for i in range(numsection):
        gu_list.append(Gu(i))
    env.gu_list = gu_list
    DQN_agent = center_DQN()
    env.DQN_agent = DQN_agent
    ...
    plt.ylabel('count')
    plt.savefig('./graph/new_graph/matching_cDQN_' + str(numsection) + 'section_' + str(second) + 'second_supply_minus_demand.png')
    ...
    plt.xlabel('episode')
    plt.ylabel('score')
    plt.savefig('./graph/new_graph/matching_cDQN_' + str(numsection) + 'section_' + str(second) + 'second_score.png')
